...
void Form::beSigned(const Bureaucrat &b) {
	if (b.getGrade() <= this->getSignRequiredGrade()) {
		if (this->_isSigned)
			throw Form::SignException("the form is already signed");
		this->_isSigned = true;
	}
	else {
...
void Form::execute(Bureaucrat const & executor) const {
	if (!this->_isSigned)
		throw Form::SignException("the form is not signed");
...
Form::SignException::SignException(std::string msg) : _msg(msg) {}
Form::SignException::~SignException() throw() {}
